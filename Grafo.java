...
public class Grafo {
    ...
    public void adicionarAresta(int peso, String inicio, String fim, String linha) {
        ...
        Vertice verticeFim = buscarVertice(fim);
        ...
        if (verticeInicio != null && verticeFim != null) {
            ...
            verticeFim.adicionarArestaSaida(arestaReversa);
            verticeInicio.adicionarArestaEntrada(arestaReversa);
...
public void DijkstraSem(String partida, String fim) {
    Vertice verticeInicio = buscarVertice(partida);
    Vertice verticeFim = buscarVertice(fim);

    if (verticeInicio == null || verticeFim == null) {
        System.out.println("Estação de partida ou chegada não encontrada.");
        return;
    }
    ...
    String[] linhaAnterior = new String[n];

    for (int i = 0; i < n; i++) {
        distancias[i] = Integer.MAX_VALUE;
        predecessores[i] = null;
        visitado[i] = false;
        linhaAnterior[i] = null;
    }
    ...
    for (int i = 0; i < n - 1; i++) {
        ...
        Vertice visitadoVertice = vertices.get(u);

        for (Aresta aresta : visitadoVertice.getArestasSaida()) {
            int v = vertices.indexOf(aresta.getFim());
            if (!visitado[v]) {
                int novaDistancia = distancias[u] + aresta.getPeso();

                String linhaNova = aresta.getLinha();
                if (linhaAnterior[u] != null && !linhaAnterior[u].equals(linhaNova)) {
                    novaDistancia += 5;
                }

                if (novaDistancia < distancias[v]) {
                    distancias[v] = novaDistancia;
                    predecessores[v] = visitadoVertice;
                    linhaAnterior[v] = linhaNova;
                }
            }
        }
    }
    ...
    for (Vertice v = verticeFim; v != null; v = predecessores[vertices.indexOf(v)]) {
        caminho.add(0, v);
    }

    if (distancias[vertices.indexOf(verticeFim)] != Integer.MAX_VALUE) {
        ...
        System.out.println("\nTempo médio do caminho: " + distancias[vertices.indexOf(verticeFim)] + " minutos.");
    } else {
    ...
    private int menorDistancia(int[] distancias, boolean[] visitado) {
    ...
    public Vertice buscarVertice(String ponta) {
        for (Vertice vertice : vertices) {
            if (vertice.getDado().equals(ponta)) {
                return vertice;
            }
        }
        return null;
